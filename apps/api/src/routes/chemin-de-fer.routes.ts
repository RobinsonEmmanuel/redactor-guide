...
export async function cheminDeFerRoutes(fastify: FastifyInstance) {
  ...
  /**
   * POST /guides/:guideId/chemin-de-fer/empty-structure
   * Initialise le chemin de fer avec N cases vides (sans template).
   * L'utilisateur peut ensuite glisser-déposer des templates depuis la palette.
   */
  fastify.post('/guides/:guideId/chemin-de-fer/empty-structure', async (request, reply) => {
    const { guideId } = request.params as { guideId: string };
    const { count = 100 } = (request.body ?? {}) as { count?: number };
    const db = request.server.container.db;

    try {
      const pageCount = Math.min(Math.max(1, Number(count)), 500);

      // 1. Upsert du document chemin de fer
      const now = new Date().toISOString();
      const cdfResult = await db.collection('chemins_de_fer').findOneAndUpdate(
        { guide_id: guideId },
        {
      ...
      // 2. Supprimer les pages existantes
      const deleteResult = await db.collection('pages').deleteMany({ chemin_de_fer_id: cheminDeFerId });
      if (deleteResult.deletedCount > 0) {
        console.log(`🗑️ [Empty Structure] ${deleteResult.deletedCount} page(s) supprimée(s)`);
      }

      // 3. Créer N cases vides
      const emptyPages = Array.from({ length: pageCount }, (_, i) => ({
        chemin_de_fer_id: cheminDeFerId,
        guide_id:         guideId,
        template_name:    null,
        template_id:      null,
      ...
      await db.collection('pages').insertMany(emptyPages);

      console.log(`✅ [Empty Structure] ${pageCount} cases vides créées pour le guide ${guideId}`);

      return reply.send({
        success: true,
        message: `${pageCount} cases vides créées`,
        chemin_de_fer_id: cheminDeFerId,
        count: pageCount,
      });
    } catch (err: any) {
      console.error('[Empty Structure] Erreur:', err.message);
