...
- Conclusion avec conseil pratique`,
  ...
  /**
   * POST /prompts/seed-validation
   * Crée (ou remplace) les prompts liés à la validation de contenu.
   * Variables disponibles : {{ARTICLE_SOURCE}}, {{NOM_POI}}, {{CHAMPS_REDIGES}}
   */
  fastify.post('/prompts/seed-validation', async (request, reply) => {
    try {
      const db = request.server.container.db;

      const PROMPT_ID = 'validation_coherence_article';

      const promptDoc = {
        prompt_id: PROMPT_ID,
        prompt_nom: 'Cohérence contenu / article source',
        intent: 'validation_coherence_article',
        categories: ['validation', 'coherence', 'article'],
        langue_source: 'fr',
        version: '1.0.0',
        actif: true,
        texte_prompt: `Tu es un éditeur vérifiant la cohérence entre un contenu rédigé et son article source.
...
Contenu rédigé pour la fiche "{{NOM_POI}}" :
{{CHAMPS_REDIGES}}
...
{ "consistency": [{ "field": "nom_du_champ", "article_consistency": "present|partial|absent", "article_excerpt": "citation courte de l'article source ou null", "article_comment": "explication max 80 caractères" }] }`,
      ...
      await db.collection('prompts').deleteOne({ prompt_id: PROMPT_ID });
      ...
      request.log.error({ error }, 'Erreur seed prompt validation');
      return reply.status(500).send({
        error: 'Erreur lors de la création du prompt de validation',
        details: error instanceof Error ? error.message : String(error),
      });
    }
  });
